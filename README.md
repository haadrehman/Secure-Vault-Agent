...
The system dynamically loads skills into the agent's context only when relevant, keeping the system prompt lean and focused. Our `.agents/skills/` directory contains:
- `ingest_doc`: Validates paths and triggers local document ingestion into ChromaDB.
- `query_vault`: Binds the `search_vault` MCP tool for retrieving relevant context.
- `redact_doc`: Enforces the local boundary rule, intercepting and masking PII before it leaves the machine.
- `deanonymize`: Reverses the placeholder tokens back to real data in the final Orchestrator response.

## Setup Instructions

1. **Python Environment**: Ensure you are using Python 3.12.12+.
2. **Install Dependencies**: We use `uv` for lightning-fast package management.
   ```bash
   uv pip install "google-adk[a2a,extensions]" chromadb presidio-analyzer pydantic pytest
   ```
...
uv run pytest tests/
```
The test suite breaks down into three core areas:
- `test_redaction.py`: PII detection accuracy across names, SSNs, and credit cards.
- `test_mcp.py`: MCP server tool registration, document ingestion, and chunk retrieval execution.
- `test_security.py`: Red-team prompt-injection resistance and path traversal guard verification.
...
- **Human-in-the-Loop (HITL) Gate**: One strict approval gate per query reviews the full redacted payload. A `LongRunningFunctionTool` pauses execution, displaying the raw vs. redacted text side-by-side for human confirmation before any data reaches the LLM.
...
- **Fallback Resilience**: A local `ollama` model (`gemma2:2b`) is designed to seamlessly take over if cloud inference fails or rate limits are reached, emitting OpenTelemetry telemetry warnings.
